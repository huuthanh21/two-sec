...
- **InterventionLifecycle**: the domain-layer module that owns the foreground and intervention session state as a sealed `SessionState` type (`Idle | InForeground(p) | PendingIntervention(p) | InIntervention(p)`). Constructed with a `BlockerEngine` and a `Clock`. Exposes `onForegroundApp(packageName): Decision`, `onInterventionShown(packageName): Unit`, and `onInterventionDismissed(packageName): Unit`. The single source of truth for the `Skip(AlreadyInForeground)` filter. Replaces `InterventionLauncher`.
- **Decision**: the return type of `BlockerEngine.decide(packageName, now)`. Sealed type with `Intervene` and `Skip(reason)` variants, where `reason` covers `MasterOff`, `NotInBlocklist`, `Whitelisted`, `IgnoredPackage`, `OwnPackage`, `AlreadyInForeground`. `AlreadyInForeground` is produced by `InterventionLifecycle` (not the engine) when the foreground package matches the current session state — it filters in-app navigation before the engine runs.
