...
- **InterventionLifecycle**: the domain-layer module that owns the foreground and intervention session state as a sealed `SessionState` type (`Idle | InForeground(p) | PendingIntervention(p) | InIntervention(p)`). Constructed with a `BlockerEngine` and a `Clock`. Exposes `onForegroundApp(packageName): Decision`, `onInterventionShown(packageName): Unit`, and `onInterventionDismissed(packageName): Unit`. The single source of truth for the `Skip(AlreadyInForeground)` filter.
